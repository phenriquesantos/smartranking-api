...
export class CategoriasService {
  ...
  async obterCategoriaPorId(id: string): Promise<Categoria>{
    const categoria = await this.categoriaModel.findOne({ _id: id }).populate('jogadores').exec();
    if(!categoria)
  ...
  async atualizarCategoria(id: string, atualizarCategoriaDto: AtualizarCategoriaDto): Promise<void>{
    await this.obterCategoriaPorId(id);
    await this.categoriaModel.findOneAndUpdate({ _id: id }, { $set: atualizarCategoriaDto }).exec();
  }
  ...
  async adicionarJogadorCategoria(params: string[]): Promise<void>{
    ...
    const categoria = await this.categoriaModel.findOne({ categoria: categoriaNome }).exec();
    if(!categoria) throw new BadRequestException(`Categoria ${categoriaNome} não encontrada`);
    ...
    await this.categoriaModel.findOneAndUpdate({ categoria: categoriaNome }, { $set: categoria }).exec();
  }
